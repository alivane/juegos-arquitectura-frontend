    ...
    helmets_all(token_item)
      ...
      .then(data => {
        ...
        for (let i = 0; i < helmets_data.length; i++) {
          if (helmets_data[i]['id'] === user['id_helmet']) setCasco(helmets_data[i])
        }
        ...
        <div className="container_option_avatar">
          <div className="container_casco">
            {
              helmetsData.map((data, key) => {
                return (
                  ...
                  >
                    <FontAwesomeIcon icon={faHardHat} color={data['path_image']} className="cascos_cambio_a" />
                  </div>
                )
              })
            }
          ...
          <div className="container_boton">
            <div className="container_casco_b">
              <span id="avatar_cambio" className="cambios_avatars"><img src={personaje['path_image']} alt="" className="avatar_principal" />
                <FontAwesomeIcon icon={faHardHat} color={casco['path_image']} className="cascos_cambio_a" />
              </span>
            </div>
          </div>

          <div className="container_boton">
            <button className="boton_avatar" onClick={() => onClickAvatar()} id="avatar_but">CAMBIAR AVATAR</button>
          </div>
              ...
              avatarsData.map((data, key) => {
                return (
                  <div key={key} className="container_avatar_a">
                    <img src={data['path_image']} onClick={() => setPersonaje(data)}
