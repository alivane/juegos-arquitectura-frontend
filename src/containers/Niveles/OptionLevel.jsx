    ...
    <>
      <div className="option_container_2">
        <div className="option_container__container">
          {
              levels.map((data, key) => {
                const unlock = levels_unlock.some(level => level['id_level'] === data['id'])
                
                if (unlock === true) {
                  return (
                    <a key={key} href={data['path']} className="option_container__a">{data['name']}</a>
                  )
                } else {
                  return (
          ...
          <a href={gender === 0 ? "profilemen" : "profilewoman"} className="option_container__a">PERFIL</a>
