...
const CoinsForm = () => {
  ...
  const updateCoin = (data) => {
    if (price >= data["coins"]) {
      ...
      helmets_by_user_register(helmets_by_user).then((response) => {
        if (!response.ok) {
          throw Error(response.statusText);
        } else {
          ...
          update_user(token_item, userUpdate, user["id"]).then(
            (response_user) => {
              if (!response_user.ok) {
      ...
      alert("No tienes Coins suficientes para comprar el casco ");
    }
  };
      ...
      <div className="container_game">
        <div className="container_coin">
          <div className="container_coin_2">
            <h1 className="price_coin">
              <span className="price_coin-2">${price}</span>
            </h1>
            <div className="icon_coin">
              <div className="container_icon_coin">
                {helmetsData.map((data, key) => {
                  ...
                  if (comprado === false) {
                    return (
                      <div key={key} className="container_helmet">
              ...
              {helmetsData.map((data, key) => {
                ...
                if (comprado === false) {
                  return (
                    <button
                      key={key}
                      className="boton_coin"
                      onClick={() => updateCoin(data)}
                    >
                      <span className="boton_coin-2">COMPRAR</span>
                    </button>
                  );
                } else {
                  return <h1 className="adquirir">Comprado</h1>;
                }
              })}{" "}
            </div>
          </div>
